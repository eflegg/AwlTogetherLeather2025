...
<div class="ajax-filters filter-by-category <?php if ($filterClass): echo $filterClass;
											else: null;
											endif; ?>">
	<form class="category-form" id="ajax-filter">
		...
		<?php foreach ($parent_cat as $catVal): ?>
			...
			<?php if ($catVal->name == $category) : ?>
				...
				<div data-type=<?php echo $dataType; ?> tax-type=<?php echo $taxonomy; ?> class="cat-select d-flex btn--green" name="categories" id="cat-select">

					<label for="all-cats" class="cat-label active btn btn--primary">
					...
					<?php foreach ($child_cat as $child_term): ?>
						<label for="<?= $child_term->slug; ?>" class="cat-label btn btn--primary">
